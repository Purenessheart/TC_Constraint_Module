function path=RRT2(q_ord,q_goal)
tree1=[];
tree1=[tree1 [q_ord;0]];
global tree2;
tree2=[];
...
while distance_p(tree1(:,n1),q_goal)>0.01%%||distance_p(tree1(:,n1),q_goal)>0.05
     if rand(1)>0.5
    ...
    if n1>1
     ...
     a1=dc;
       ...
       while a1>0.1
           ...
           v=[v(1:3);0;0;0];
            nodle_x=nodle_et(q_near.x,v);
%              nodle=nislotion1(nodle_x)
            nodle=nislotion2(nodle_x,tree1(1:6,b));
              c=0;
%               if nodle(1)==100
%                   break
%               end
